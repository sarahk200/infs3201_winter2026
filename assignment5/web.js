...
const documentStorage = multer.diskStorage({
    destination: function (req, file, cb) {
        let uploadDir = persistence.getEmployeeUploadDir(req.params.id)
        cb(null, uploadDir)
    },
    filename: function (req, file, cb) {
...
app.post("/login", async (req, res) => {
    ...
    let loginResult = await business.validateLoginAndSetup2FA(username, password)

    if (!loginResult.success) {
        res.render("login", { error: loginResult.error, message: "" })
        return
    }

    pending2FA[username] = {
        code: loginResult.twoFACode,
        expiry: Date.now() + 3 * 60 * 1000
    }

    res.setHeader("Set-Cookie", "pendingUser=" + username)
    res.redirect("/2fa")
})
...
app.get("/2fa", (req, res) => {
    ...
    if (req.headers.cookie) {
        ...
        for (let i = 0; i < cookies.length; i++) {
            let cookie = cookies[i].trim()
            if (cookie.startsWith("pendingUser=")) {
                pendingUser = cookie.split("=")[1]
            }
        }
    }

    if (!pendingUser || !pending2FA[pendingUser]) {
        res.redirect("/login")
        return
    }

    res.render("twofa", { error: null })
})
...
app.post("/2fa", async (req, res) => {
    ...
    let data = pending2FA[pendingUser]
    let result = business.validateTwoFactorCode(data.code, submittedCode, data.expiry)

    if (!result.valid) {
        if (result.error.includes("expired")) {
            delete pending2FA[pendingUser]
            res.setHeader("Set-Cookie", "pendingUser=; Max-Age=0")
            res.render("login", { error: "Your verification code expired. Please log in again.", message: "" })
            return
        }

        res.render("twofa", { error: result.error })
        return
    }

    let sessionKey = await business.startSession(pendingUser)
    res.setHeader("Set-Cookie", [
    "sessionkey=" + sessionKey,
    "pendingUser=; Max-Age=0"
    ])
    res.redirect("/")
})
/**
...
app.get("/employees/:id/documents", checkAuth, async (req, res) => {
    ...
    let documents = persistence.getEmployeeDocuments(employeeId)
    res.render("documents", { employee: employee, documents: documents, error: null, success: null })
})

app.post("/employees/:id/documents", checkAuth, documentUpload.single("document"), async (req, res) => {
    let employeeId = req.params.id
    let employee = await business.getEmployee(employeeId)
    ...
    let existingDocs = persistence.getEmployeeDocuments(employeeId)
    let validationResult = business.validateDocumentUpload(req.file, existingDocs)
    ...
    if (!validationResult.valid) {
        ...
        let documents = persistence.getEmployeeDocuments(employeeId)
        res.render("documents", { employee: employee, documents: documents, error: validationResult.error, success: null })
        return
    }

    let documents = persistence.getEmployeeDocuments(employeeId)
    res.render("documents", { employee: employee, documents: documents, error: null, success: "Document uploaded successfully!" })
})

app.get("/employees/:id/documents/:filename", checkAuth, (req, res) => {
    let employeeId = req.params.id
    let filename = req.params.filename

    let filePath = persistence.getDocumentPath(employeeId, filename)
 ...
 * @param {Object} res
 */
app.post('/photo/:empId', checkAuth, photoUpload.single('photo'), async (req, res) => {
    let empId = req.params.empId
    if (!req.file) {
